...
static bool areUnique(ArrayAttr flags) {
...
LogicalResult UnaryDispatchOp::verify() {
  if (!areUnique(getFlags()))
    return emitOpError() << "expected" << FLAGS_NAME << " to be unique";
  // 'inputs' = [m, n, lda, ldo]
...
LogicalResult FusedBrgemmDispatchOp::verify() {
  if (!areUnique(getBinaryFlags()))
    return emitOpError() << "expected " << BINARY_FLAGS_NAME << " to be unique";
  if (!areUnique(getUnaryFlags()))
    return emitOpError() << "expected " << UNARY_FLAGS_NAME << " to be unique";
  // 'inputs' = [m, n, k, lda, ldb, ldc]
  return verifyGemmLikeOp<FusedBrgemmDispatchOp>(*this);
}
