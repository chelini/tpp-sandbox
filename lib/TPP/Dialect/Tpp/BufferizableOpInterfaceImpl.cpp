...
struct MulBufferizationInterface
    : public BufferizableOpInterface::ExternalModel<MulBufferizationInterface,
                                                    tpp::MulOp> {
  ...
  LogicalResult bufferize(Operation *op, RewriterBase &rewriter,
                          const BufferizationOptions &options) const {
    return bufferizeBinaryOp<tpp::MulOp>(op, rewriter, options);
  }
};
...
void registerBufferizableOpInterfaceExternalModels(DialectRegistry &registry) {
  registry.addExtension(+[](MLIRContext *ctx, tpp::TppDialect *dialect) {
    ...
    MulOp::attachInterface<tpp::MulBufferizationInterface>(*ctx);
