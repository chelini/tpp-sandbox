...
transform::PackOpExt::applyToOne(transform::TransformRewriter &rewriter, linalg::LinalgOp target,
...
DiagnosedSilenceableFailure
transform::CollapseOp::applyToOne(transform::TransformRewriter &rewriter, linalg::LinalgOp target,
                                  ApplyToEachResultList &results,
                                  transform::TransformState &state) {
...
DiagnosedSilenceableFailure
transform::RewriteToBrgemmOp::applyToOne(transform::TransformRewriter &rewriter, linalg::LinalgOp target,
                                         ApplyToEachResultList &results,
                                         transform::TransformState &state) {
...
DiagnosedSilenceableFailure
transform::RewriteConvToMatmulOp::applyToOne(transform::TransformRewriter &rewriter, 
...
DiagnosedSilenceableFailure transform::FoldUnitExtentDimsOp::applyToOne(transform::TransformRewriter &rewriter,
    Operation *target, ApplyToEachResultList &results, TransformState &state) {
  if (!target->hasTrait<OpTrait::IsIsolatedFromAbove>()) {
    auto diag = this->emitOpError("requires isolated-from-above targets");
    diag.attachNote(target->getLoc()) << "non-isolated target";
...
DiagnosedSilenceableFailure
transform::GetBlockedConvolutions::apply(transform::TransformRewriter &rewriter, transform::TransformResults &results,
                                         transform::TransformState &state) {
...
DiagnosedSilenceableFailure
transform::GetBlockedMatmuls::apply(transform::TransformRewriter &rewriter, transform::TransformResults &results,
                                    transform::TransformState &state) {
  SmallVector<Operation *> res;
  auto payloadOps = state.getPayloadOps(getTarget());
  for (Operation *op : payloadOps) {
